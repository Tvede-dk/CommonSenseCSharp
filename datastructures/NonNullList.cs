...
namespace CommonSenseCSharp.datastructures {
    ...
    public class NonNullList<T> : List<T> {
        ...
        public new void AddRange([CanBeNull] IEnumerable<T> collection) => collection?.FlatForeach(Add);
        ...
        /// <returns></returns>
        [Pure]
        public new bool Contains([CanBeNull] T item) => item != null && base.Contains(item);

        [NotNull]
        public static NonNullList<T> CreateFrom([NotNull] params T[] items) => items.FlatMap(x => x);


        [NotNull]
        public static NonNullList<T> CreateFrom([NotNull] T item, [NotNull] IEnumerable<T> otherLists)
